...
def uniformCostSearch(problem):
    ...
    while not priorityQueue.isEmpty():
        (state, visited, cost) = priorityQueue.pop()
        if problem.isGoalState(state):
            return visited
        if not state in path:
            ...
            for successor, action, actionCost in problem.getSuccessors(state):
                ...
                priorityQueue.push((successor, visited + [action], g()), g())
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    h = heuristic
    path = []
    priorityQueue.push((problem.getStartState(), [], 0), h(state, problem))
    while not priorityQueue.isEmpty():
        (state, visited, cost) = priorityQueue.pop()
        if problem.isGoalState(state):
            return visited
        if not state in path:
            ...
            for successor, action, actionCost in problem.getSuccessors(state):
                def g():
                    return cost + actionCost
                priorityQueue.push((successor, visited + [action], g()), g() + h(successor, problem))
